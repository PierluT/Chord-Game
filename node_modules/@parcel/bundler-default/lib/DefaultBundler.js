...
function createIdealGraph(assetGraph, config, entries) {
  ...
  for (let asset of assets) {
    ...
    for (let a of reachable) {
      if (entries.has(a) || !a.isBundleSplittable || getBundleFromBundleRoot(a).needsStableName || getBundleFromBundleRoot(a).bundleBehavior === 'isolated') {
  ...
  function deleteBundle(bundleRoot) {
    ...
    if (reachableRoots.hasContentKey(bundleRoot.id)) {
      reachableRoots.replaceNodeIdsConnectedTo(reachableRoots.getNodeIdByContentKey(bundleRoot.id), []);
    }
  ...
  function mergeBundle(mainNodeId, otherNodeId) {
    ...
    let bundleGroupOfMain = (0, _nullthrows().default)(bundleRoots.get(mainBundleRoot))[1];
